        ...
        <div class="flex-wrap container-fluid d-flex align-items-center justify-content-between flex-sm-nowrap">
            ...
            <div class="flex-wrap mr-1 d-flex align-items-center">
                ...
                <div class="flex-wrap mr-5 d-flex align-items-baseline">
                    <!--begin::Page Title-->
                    <h5 class="my-1 mr-5 text-dark font-weight-bold">Редактировать транспорт</h5>
                    <!--end::Page Title-->
                    <!--begin::Breadcrumb-->
		...
		<form action="{{route('admin.transport.update')}}" method="post">
			...
			<div class="form-group">
				...
				<select class="form-control" name="type" id="type">
					@if(isset($transport))
						@if($transport->type == 1)
							<option value="1" selected>Жд</option>
							<option value="2">Авиа</option>
							<option value="3">Авто</option>
						@elseif($transport->type == 2)
							<option value="1" >Жд</option>
							<option value="2" selected>Авиа</option>
							<option value="3">Авто</option>
						@else
							<option value="1" >Жд</option>
							<option value="2" >Авиа</option>
							<option value="3" selected>Авто</option>
						@endif	
					@endif
			...
			<div class="form-group">
				...
				<input type="text" name="name" id="name" class="form-control" 
					   placeholder="Введите название транспортной компаний" value="{{$transport->name}}">
			</div>
			<div class="form-group">
				<label for="phone">Телефон</label>
				<input type="text" name="phone" id="phone" class="form-control" 
					   placeholder="Введите телефон ответственного " value="{{$transport->phone}}">
			</div>
			<div class="form-group">
				<label for="manager">Менеджер</label>
				<input type="text" name="manager" id="manager" class="form-control" 
					   placeholder="Введите имя ответственного " value="{{$transport->manager}}">
			</div>
			<div class="form-group">
				<label for="email">Email</label>
				<input type="text" name="email" id="email" class="form-control" 
					   placeholder="Введите email ответственного " value="{{$transport->email}}">
			</div>
			<div class="form-group">
