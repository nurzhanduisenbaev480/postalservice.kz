...
@section('content-admin')
    <div class="mx-auto col-lg-12">
        <div class="container-fluid">
            <div class="row">
                <div class="col-md-12">
                    <a href="{{route('admin.store.summary')}}" class="btn btn-primary">Назад</a>
                </div>
            </div>
            <div class="row mt-5">
                <div class="col-md-4">
                    <div class="summary_info">
                        ...
                        <ul class="summary">
                            <li>
                                <div class="summary_key">Описание:</div>
                                <div class="summary_value">{{$summary->description}}</div>
                            </li>
                            <li>
                                <div class="summary_key">Статус:</div>
                                <div class="summary_value">{{$summary->status}}</div>
                            </li>
                            <li>
                            ...
                            <tbody>
                                @if(isset($overheads))
                                    ...
                                    <tr>
                                        ...
                                        @if (\App\Models\Journal::where('overhead_code', $overhead->overhead_code)
											->orderBy('id', 'DESC')->get()->first()->status_id != 18 && \App\Models\Journal::where('overhead_code', $overhead->overhead_code)
											->orderBy('id', 'DESC')->get()->first()->status_id != 8)
                                            <td>
                                                <a href="{{route('admin.store.yesOverhead')}}?id={{$overhead->id}}
														 &summary_id={{$summary->id}}" 
												   class="btn btn-danger">Принять</a>
                                            </td>
                                        @else
                                            <td>Принято</td>
                                        @endif
										@if(\App\Models\Journal::where('overhead_code', $overhead->overhead_code)
											->orderBy('id', 'DESC')->get()->first()->status_id == 18)
											<td class="" style="background:green;font-size:14px;color:white;">
												Принято на складе 
										...
										@elseif(\App\Models\Journal::where('overhead_code', $overhead->overhead_code)
											->orderBy('id', 'DESC')->get()->first()->status_id == 8)
										<td class="" style="background:green;font-size:14px;color:white;">
											Доставлен
										</td>
